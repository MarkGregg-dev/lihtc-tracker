...
export function DocsTab({ project }) {
  ...
  async function openDoc(doc) {
    ...
    try {
      ...
      let href
      if (isExcel) {
        const pub = 'https://iohcoankgpokhhldvzvy.supabase.co/storage/v1/object/public/project-docs/' + doc.storage_path.split('/').map(p => encodeURIComponent(p)).join('/')
        href = 'https://view.officeapps.live.com/op/view.aspx?src=' + encodeURIComponent(pub)
      } else {
  ...
  function guessType(filename) {
    ...
    if (f.includes('financial') || f.includes('financials')) return 'pm-report'
    ...
    <div>
      ...
      <div style={{ background: '#eceae3', borderRadius: 8, padding: '10px 14px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: '#6b6a63' }}>PDF, Excel, Word up to 50MB. Multiple files OK.</div>
        </div>
        ...
        {!addOpen ? (
          ...
          <div style={{ background: '#fff', border: '0.5px solid #888780', borderRadius: 8, padding: '12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
              {[['Name','name','text','PM Financials May 2026'],['Folder','folder','select',null],['Type','doc_type','select2',null],['Notes','notes','text','Optional']].map(([label, key, type, ph]) => (
            ...
            <div style={{ marginBottom: 10 }}>
              <label style={{ fontSize: 11, color: '#6b6a63', display: 'block', marginBottom: 4 }}>File (optional)</label>
